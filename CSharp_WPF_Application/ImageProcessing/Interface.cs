...
namespace ImageProcessing
{
    ...
    public class D_FirstStation
    {
        ...
        // Dilation
        public Image<Gray, byte> ToDilation(Image<Gray, byte> OriginImage, int KernelSize = 3, int Iterations = 1)
        {
            Mat kernel = CvInvoke.GetStructuringElement(ElementShape.Rectangle, new System.Drawing.Size(KernelSize, KernelSize), new System.Drawing.Point(-1, -1));
            Image<Gray, byte> result = new Image<Gray, byte>(OriginImage.Size);
            CvInvoke.Dilate(OriginImage, result, kernel, new System.Drawing.Point(-1, -1), Iterations, BorderType.Default, new MCvScalar(0));
            return result;
        }

        // Erosion
        public Image<Gray, byte> ToErosion(Image<Gray, byte> OriginImage, int KernelSize = 3, int Iterations = 1)
        {
            Mat kernel = CvInvoke.GetStructuringElement(ElementShape.Rectangle, new System.Drawing.Size(KernelSize, KernelSize), new System.Drawing.Point(-1, -1));
            Image<Gray, byte> result = new Image<Gray, byte>(OriginImage.Size);
            CvInvoke.Erode(OriginImage, result, kernel, new System.Drawing.Point(-1, -1), Iterations, BorderType.Default, new MCvScalar(0));
            return result;
        }
